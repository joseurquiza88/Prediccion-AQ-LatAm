...
# Funcion 01 con coef. de angstroment ----
...
# ---------------------------------------------------------------
# Funcion 02 con funcion de interpolacion cuadratica ----
...
interpolate_s <- function(path, path_write){
  ...
  # for (i in 1:length(archive)){
  for (i in 1:seq_along(archive)){
    print(i)
    data <- read.csv (archive[i],  header = TRUE, sep = ",", dec = ".", stringsAsFactors = FALSE, na.strings = "-999")
    date <- strptime(paste (data$Date.dd.mm.yyyy., data$Time.hh.mm.ss., sep = " "), format = "%d:%m:%Y %H:%M", tz = "GMT")  # nolint
    name <- substr (archive[i], 1, 12)
    x <- log(550) #Valor desconocido
